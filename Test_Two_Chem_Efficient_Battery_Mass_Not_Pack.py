...
def Two_Chem_Efficient_Battery_Mass_Not_Pack(battery_1, battery_2, req_capacity, peak_power_req, max_pack_V_allowed, min_pack_V_allowed, max_mass, peak_charge_power_req):
    ...
    check_capacity, capacity = Check_Capacity(battery_1_Wh, battery_2_Wh, no_battery_1_series, no_battery_2_series, no_battery_1_parallel, no_battery_2_parallel, req_capacity)

    while check_max_V == 0 and check_mass == 1 and check_min_V == 1 and check_capacity == 1:

        ############################################################################

        # Got to here on putting check_capacity in
        ...
        if check_min_V == 0 or check_capacity == 0:
            success = 0
            return success, no_battery_1_series, no_battery_1_parallel, no_battery_2_series, no_battery_2_parallel, capacity, 0, mass, 0
    ...
    while peak_power_req > peak_power_generated and check_mass == 1 and check_max_V == 1:
        ...
        if check_mass == 0 or check_max_V == 0 or check_min_V == 0 or check_capacity == 0:
            print(f"Mass, voltage or capacity over limit{mass, max_pack_V, min_pack_V, capacity}")
            success = 0
            return success, no_battery_1_series, no_battery_1_parallel, no_battery_2_series, no_battery_2_parallel, capacity, 0, mass, 0
